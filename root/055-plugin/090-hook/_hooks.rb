  ...
  class DocDefiner
    ...
    def klass_to_name(klass)
      if klass.kind_of?(String) && klass =~ /\Ajs:(.+)\z/
        "[node:plugin/interface/#{$1.gsub(/([a-z0-9])([A-Z])/,'\\1-\\2').downcase}]"
      elsif klass == Symbol
        "@String@"
      elsif klass == "bool"
        "boolean"
        ...
        "[node:plugin/interface/ref]"
      elsif klass == KObject
        "[node:plugin/interface/store-object]"
      elsif klass == "KObjectMutable"
        "[node:plugin/interface/store-object-mutable]"
      elsif klass == WorkUnit
        "[node:plugin/interface/work-unit]"
      elsif klass == AuditEntry
        "[node:plugin/interface/audit-entry]"
      elsif klass == User
        "[node:plugin/interface/security-principal]"
      elsif klass == KLabelList
        "[node:plugin/interface/label-list]"
      elsif klass == KLabelChanges
        "[node:plugin/interface/label-changes]"
      elsif klass == KLabelStatements
        "[node:plugin/interface/label-statements]"
      elsif klass == StoredFile
        "[node:plugin/interface/file]"
      elsif klass == "JSRequest"
        "[node:plugin/interface/request]"
      elsif klass == Hash
